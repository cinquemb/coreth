...
func (pool *TxPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*txSortedMap) {
	...
	if reset != nil {
		...
		if reset.newHead != nil && pool.chainconfig.IsApricotPhase3(new(big.Int).SetUint64(reset.newHead.Time)) {
			_, baseFeeEstimate, err := dummy.CalcBaseFee(pool.chainconfig, reset.newHead, uint64(aclock.Now().Unix()))
			if err == nil {
...
func (pool *TxPool) startPeriodicFeeUpdate() {
	...
	if aclock.Now().After(time.Unix(pool.chainconfig.ApricotPhase3BlockTimestamp.Int64(), 0)) {
		pool.updateBaseFee()
	}
...
func (pool *TxPool) updateBaseFee() {
	...
	_, baseFeeEstimate, err := dummy.CalcBaseFee(pool.chainconfig, pool.currentHead, uint64(aclock.Now().Unix()))
	if err == nil {
		pool.priced.SetBaseFee(baseFeeEstimate)
	} else {
